...
// 比較関数を定義
bool compareEmails(const std::string& email1, const std::string& email2) {
	// "ｇ"の後の４桁の数字を取り出して比較
	std::string suffix1 = email1.substr(email1.find('g') + 1, 4);
	std::string suffix2 = email2.substr(email2.find('g') + 1, 4);
...
int main() {
	...
	// ソート処理
	for (int i = 0; i <= 4; i++) {
		std::sort(adress.begin(), adress.end(), compareEmails);
	}
